...
void push(struct proc_queue *queue, struct proc process) {
	...
	int pos = queue->rear;
	while (pos >= 0 && queue->processes[pos].priority > process.priority) {
    	queue->processes[pos + 1] = queue->processes[pos];
    	pos--;
	}
	queue->processes[pos + 1] = process;
	queue->rear++;
}
...
void printQueue(struct proc_queue *queue) {
	...
	for (int i = queue->front; i <= queue->rear; i++) {
    	printf("Name: %s, Priority: %d, Memory: %d, Runtime: %d\n", queue->processes[i].name, queue->processes[i].priority, queue->processes[i].memory, queue->processes[i].runtime);
	}
}
